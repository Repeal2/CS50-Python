...
The exact JSON a WhisperX worker expects/returns depends on which one you deploy — see _build_payload and
_parse_segments, the two places that would need adjusting for a different template's schema than the one
guessed at here (loosely modeled on the common runpod-workers/worker-whisperx shape).
"""
...
# started, but Runpod's serverless queue API caps how big one job's request body can be. Above this, a
# real upload target + presigned URL (see RunpodWhisperXTranscriber's `upload` argument) is the only
# option. Runpod doesn't publish an exact number for this, so it's a deliberately cautious guess rather
# than a tight limit — the failure mode of guessing too high (a rejected request) is worse than guessing
# too low (falling back to local transcription a bit earlier than strictly necessary).
...
def _build_payload(audio_url: str, *, huggingface_token: str | None) -> dict:
    """The input schema a runpod-workers/worker-whisperx-style endpoint expects. Adjust field names here
    to match whatever WhisperX worker image actually gets deployed — this is the one place that depends
    on it."""
    payload = {"input": {"audio": audio_url, "model": "large-v3", "diarization": True}}
